...
app.put('/clients/:id', authenticateToken, async (req, res) => {
  ...
  try {
    const sql = 'UPDATE capstone.clients SET name = ?, email = ?, phone_number = ?, service = ? WHERE client_id = ?';
    const [result] = await connection.promise().query(sql, [name, email, phone_number, service, clientId]);

    if (result.affectedRows === 0) {
      return res.status(404).json({ message: 'Client not found' });
    }
    res.status(200).json({ message: 'Client updated successfully', clientId: clientId });
  } catch (err) {
